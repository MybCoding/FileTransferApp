...
namespace FileTransferApp
{
    public partial class MainPage : ContentPage
    {
        ...
        private string _statusMessage = LocalizationResourceManager.T("ReadyToDiscover");
        ...
        public string LanServerButtonText => IsLanServerRunning
            ? LocalizationResourceManager.T("StopLanLink")
            : LocalizationResourceManager.T("StartLanLink");
        ...
        public MainPage()
        {
            ...
            OverlayVersionLabel.Text = LocalizationResourceManager.T("Version", AppInfo.Current.VersionString);
        ...
        private async Task DoRefreshAsync()
        {
            try
            {
                ...
                _hiddenIps.Clear();
                SaveHiddenIps();
                StatusMessage = LocalizationResourceManager.T("Refreshing");
                for (int i = 0; i < 2; i++)
                {
                    await _discoveryService.BroadcastPresenceAsync(DeviceInfo.Name, _cts?.Token ?? CancellationToken.None);
                    await Task.Delay(400);
                }
                StatusMessage = LocalizationResourceManager.T("Refreshed");
            }
            catch (Exception ex)
            {
                StatusMessage = LocalizationResourceManager.T("RefreshError", ex.Message);
            }
            finally
            {
                IsRefreshing = false;
        ...
        private async Task DeleteDeviceAsync(DeviceModel device)
        {
            ...
            var ok = await DisplayAlert(LocalizationResourceManager.T("DeleteDeviceTitle"),
                LocalizationResourceManager.T("DeleteDeviceBody", device.Name, ip),
                LocalizationResourceManager.T("Delete"),
        ...
        private async Task NavigateToChatPage(DeviceModel device)
        {
            ...
            {
                await DisplayAlert(LocalizationResourceManager.T("NavigationError"),
                    LocalizationResourceManager.T("NavErrorBody", ex.Message),
                    LocalizationResourceManager.T("OK"));
            }
        }
        ...
        private async Task StartPairingAsync(DeviceModel device)
        {
            ...
            {
                await DisplayAlert(LocalizationResourceManager.T("Error"), LocalizationResourceManager.T("PairingMissingIp"), LocalizationResourceManager.T("OK"));
                return;
            }

            StatusMessage = LocalizationResourceManager.T("PairingWith", device.Name);
            bool ok = false;
            try
            {
            ...
            {
                StatusMessage = LocalizationResourceManager.T("PairingError", ex.Message);
                return;
            }

            // PairingCompleted also reports the result (responder + initiator);
            // this covers failures that abort before the event is raised.
            if (!ok) StatusMessage = LocalizationResourceManager.T("PairingFailed", device.Name);
        }
        ...
        private async void OnPairingPrompt(string ip, string peerDeviceId, string peerName, string sas, string sessionId)
        {
            try
            {
                ...
                var accepted = await DisplayAlert(
                    LocalizationResourceManager.T("ConfirmPairing"),
                    LocalizationResourceManager.T("ConfirmPairingBody", peerName, ip, sas),
                    LocalizationResourceManager.T("TrustThisDevice"),
        ...
        private void OnPairingCompleted(string peerDeviceId, string peerName, bool success)
        {
            ...
            StatusMessage = success
                ? LocalizationResourceManager.T("Paired", peerName)
                : LocalizationResourceManager.T("PairingFailedOrCancelled", peerName);
        ...
        private async void OnLanServerToggle(object sender, EventArgs e)
        {
            try
            {
                if (!IsLanServerRunning)
                {
                    ...
                    var verText = string.IsNullOrWhiteSpace(version) ? "" : LocalizationResourceManager.T("Version", version);
                    var extra = string.IsNullOrWhiteSpace(downloadUrl)
                        ? LocalizationResourceManager.T("DownloadNotConfigured")
                        : string.Empty;
                    await DisplayAlert(LocalizationResourceManager.T("LanLinkReady"),
                        LocalizationResourceManager.T("LanLinkReadyBody", verText, LanServerUrl, extra),
                        LocalizationResourceManager.T("OK"));
                }
                else
                {
            ...
            {
                await DisplayAlert(LocalizationResourceManager.T("LanLinkError"), ex.Message, LocalizationResourceManager.T("OK"));
            }
        }
        ...
        private async void OnCopyLanUrl(object sender, EventArgs e)
        {
            try
            {
                if (!string.IsNullOrWhiteSpace(LanServerUrl))
                {
                    ...
                    await DisplayAlert(LocalizationResourceManager.T("Copied"),
                        LocalizationResourceManager.T("AddressCopied"),
                        LocalizationResourceManager.T("OK"));
                }
            }
            catch (Exception ex)
            {
                await DisplayAlert(LocalizationResourceManager.T("CopyError"), ex.Message, LocalizationResourceManager.T("OK"));
            }
        }

        private async void OnLanguageTapped(object sender, EventArgs e)
        {
            string enLabel = LocalizationResourceManager.T("English");
            string faLabel = LocalizationResourceManager.T("Persian");

            var choice = await DisplayActionSheet(
                LocalizationResourceManager.T("Language"),
                LocalizationResourceManager.T("Cancel"),
                null,
                LocalizationResourceManager.T("English"),
                LocalizationResourceManager.T("Persian"));

            if (choice == enLabel && LocalizationResourceManager.Instance.CurrentLanguage != "en")
            {
                LocalizationResourceManager.Instance.SetLanguage("en");
            }
            else if (choice == faLabel && LocalizationResourceManager.Instance.CurrentLanguage != "fa")
            {
                LocalizationResourceManager.Instance.SetLanguage("fa");
            }
            else
            {
                return;
