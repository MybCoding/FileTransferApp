    ...
    public partial class MessageModel : ObservableObject
    {
        ...
        public string FileSizeDisplay
        {
            get
            {
                ...
                if (b >= K * K * K) return LocalizationResourceManager.T("UnitGB", b / (K * K * K));
                if (b >= K * K) return LocalizationResourceManager.T("UnitMB", b / (K * K));
                if (b >= K) return LocalizationResourceManager.T("UnitKB", b / K);
                return LocalizationResourceManager.T("UnitBytes", FileSizeBytes);
            }
        }
        ...
        public string SpeedDisplay
        {
            get
            {
                if (IsPaused) return LocalizationResourceManager.T("TransferStatusPaused");
        ...
        public string TransferStatusText
        {
            get
            {
                return State switch
                {
                    TransferState.Sending => LocalizationResourceManager.T("TransferStatusSending"),
                    TransferState.Receiving => LocalizationResourceManager.T("TransferStatusReceiving"),
                    TransferState.Paused => LocalizationResourceManager.T("TransferStatusPaused"),
                    TransferState.Completed => LocalizationResourceManager.T("TransferStatusCompleted"),
                    TransferState.Failed => LocalizationResourceManager.T("TransferStatusFailed"),
                    TransferState.Canceled => LocalizationResourceManager.T("TransferStatusCanceled"),
                    TransferState.Queued => LocalizationResourceManager.T("TransferStatusQueued"),
                    _ => string.Empty
                };
            }
        }
        ...
        {
            const double K = 1024d;
            if (bytesPerSec >= K * K * K) return LocalizationResourceManager.T("SpeedGB", bytesPerSec / (K * K * K));
            if (bytesPerSec >= K * K) return LocalizationResourceManager.T("SpeedMB", bytesPerSec / (K * K));
            if (bytesPerSec >= K) return LocalizationResourceManager.T("SpeedKB", bytesPerSec / K);
            return LocalizationResourceManager.T("SpeedB", bytesPerSec);
        }
        ...
        private static string FormatSize(long bytes)
        {
            ...
            if (b >= K * K) return LocalizationResourceManager.T("UnitMB", b / (K * K));
            if (b >= K) return LocalizationResourceManager.T("UnitKB", b / K);
            return LocalizationResourceManager.T("UnitBytes", bytes);
        }
    }
}